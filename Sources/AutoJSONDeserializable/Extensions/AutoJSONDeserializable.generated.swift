...
extension JSONDeserializableProperty: JSONDeserializable {
    internal init?(JSONObject: [String: Any]) {
        guard
        let entity = (JSONObject["entity"] as? [String: Any]).flatMap(Entity.init(JSONObject:))
        else { return nil }
        self.entity = entity
        ...
        guard
        let annotatedEntity = (JSONObject["annotated_entity"] as? [String: Any]).flatMap(Entity.init(JSONObject:))
        else { return nil }
...
extension MultiTypesProperties: JSONDeserializable {
    internal init?(JSONObject: [String: Any]) {
        ...
        self.string = string
        guard
        let integer = JSONObject["integer"] as? Int
        else { return nil }
...
extension SinglePropertyWithKeyPathAnnotation: JSONDeserializable {
    internal init?(JSONObject: [String: Any]) {
        guard
        let name = JSONObject["label"] as? String
        else { return nil }
        self.name = name
    }
}
